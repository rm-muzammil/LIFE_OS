...
import { callGemini, getUserGeminiKeys, GeminiError } from '@/lib/gemini';
...
export async function GET() {
  try {
    ...
    const rows = await db
      .select()
      .from(chatMessages)
      .where(and(eq(chatMessages.userId, userId), gte(chatMessages.date, cutoff)))
...
export async function POST(req: NextRequest) {
  try {
    ...
    const [userMsg] = await db
      .insert(chatMessages)
      .values({ userId, role: 'user', content: body.content.trim(), date, feedsSchedule })
      .returning();
    ...
    const todayRows = await db
      .select()
      .from(dailySchedule)
      .where(and(eq(dailySchedule.userId, userId), gte(dailySchedule.date, date)))
      .limit(1);
...
Reply to the latest user message now, as the assistant, in plain text (no markdown, no JSON).`;
    ...
    try {
      ...
      replyText = (await callGemini(prompt, { temperature: 0.6, apiKeys })).trim();
    } catch (err) {
      replyText =
    ...
    const [assistantMsg] = await db
      .insert(chatMessages)
      .values({ userId, role: 'assistant', content: replyText, date, feedsSchedule: false })
      .returning();
