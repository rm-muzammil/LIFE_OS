...
export async function GET() {
  try {
    const rows = await db.select().from(prayerTimes).limit(1);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const existing = await db.select().from(prayerTimes).limit(1);

    if (existing[0]) {
      const [updated] = await db
        .update(prayerTimes)
        .set({ ...body, updatedAt: new Date() })
        .where(eq(prayerTimes.id, existing[0].id))
        .returning();
      return NextResponse.json(updated);
    }

    const [created] = await db.insert(prayerTimes).values(body).returning();
